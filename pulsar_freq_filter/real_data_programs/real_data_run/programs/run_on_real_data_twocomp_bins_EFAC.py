...
def run(params):
    ...
    samples_tempo = result.posterior.to_numpy()[:, :8].copy()
    samples_tempo[:, 0:4] = np.log10(samples_tempo[:, 0:4])
    samples_tempo[:, 6:8] = np.log10(samples_tempo[:, 6:8])
    ...
    labels = ['$r$', '$\\tau$ $[\\rm{s}]$',
              ...
              '$\\langle \\dot\\Omega_c\\rangle$ $[\\rm{rad~s^{-2}}]$', 'EFAC', 'EQUAD']
    fig = corner(samples_tempo, bins = 50, 
                 range = [(np.log10(r_min), np.log10(r_max)), (np.log10(tau_min), np.log10(tau_max)),
                          ...
                          (np.log10(EQUAD_min), np.log10(EQUAD_max))],
