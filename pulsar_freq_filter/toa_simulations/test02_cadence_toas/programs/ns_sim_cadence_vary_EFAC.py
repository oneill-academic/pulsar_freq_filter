...
import sys
import libstempo
from simulate_toas_cadence import simulate_toas_cadence
...
def run(params):
    ...
    if not params.resume_run:
        #Load omgc_dot and omgc_0
        newpsr = libstempo.tempopulsar(parfile=params.parfile, timfile=params.timfile)
        indexs = newpsr.pets().argsort()
        mytimes = newpsr.pets()[indexs]
        pets0 = mytimes[0].astype(np.float64)
        omgc_0 = 2*np.pi*newpsr['F0'].val
        omgc_dot = (2*np.pi*newpsr['F1'].val).astype(np.float64)
        omgc_0 = (omgc_0 + omgc_dot*(pets0 - newpsr['PEPOCH'].val)*86400).astype(np.float64)
        ...
        priors['omgc_dot'] = bilby.core.prior.DeltaFunction(omgc_dot, name='omgc_dot', latex_label='$\\langle \\dot{\\Omega}_c \\rangle$')
        priors['omgc_0'] = bilby.core.prior.DeltaFunction(omgc_0, name='omgc_0', latex_label='$\\Omega_{\\rm{c}, 0}$')

        vals = priors.sample()
        tauc, taus, Qc, Qs, Nc, Ns = param_map(vals)
        ...
        #Set the number of TOAs used for a frequency fit
        Tfit = params.Tfit
        ...
        simulate_toas_cadence(tau=tau, r=r, sigmac=sigmac, sigmas=sigmas, lag=lag, phi_0=phi_0, 
                              input_parfile=params.parfile, input_timfile=params.timfile, 
                              output_parfile=new_parfile, output_timfile=true_timfile, 
                              output_freqfile=true_freqfile, out_directory=params.out_directory, 
                              tag=params.tag)
        ...
        mcmc_params = {'r': taus/tauc, 'tau': tau, 'Qc': Qc, 'Qs': Qs, 'lag': lag, 'omgc_dot': omgc_dot, 'omgc_0': omgc_0,
                       ...
                       'R_out': R_out, 'Tfit': Tfit, 'Nfit_min': Nfit_min, 'pets0': pets0, 
                       ...
                       'sim_lag_min': sim_lag_min, 'sim_lag_max': sim_lag_max}
...
if __name__=="__main__":
    ...
    #Data files
    parser.add_argument("--parfile", default=None, type=str, help="par file for simulating real data cadence")
    parser.add_argument("--timfile", default=None, type=str, help="tim file for simulating real data cadence")
    ...
    parser.add_argument("--threshold", help="cut freq residuals greater than threshold", default=1e-8, type=float)
