...
def simulate_toas_cadence(tau, r, sigmac, sigmas, lag, phi_0,
                          input_parfile, input_timfile, output_parfile, 
                          output_timfile, output_freqfile, out_directory, tag):
    ...
    print('Simulate TOAs from known PSR')
    newpsr = libstempo.tempopulsar(parfile=input_parfile, timfile=input_timfile)
    indexs = newpsr.pets().argsort()
    mytimes = newpsr.pets()[indexs]
    toa_errors = newpsr.toaerrs[indexs]*1e-6
    Nobs = mytimes.size
    pets0 = mytimes[0]
    tstarts = mytimes*86400
    omgc_0 = 2*np.pi*newpsr['F0'].val
    omgc_dot = 2*np.pi*newpsr['F1'].val
    omgc_0 = omgc_0 + omgc_dot*(pets0 - newpsr['PEPOCH'].val)*86400
    ...
    #Original TOAs compared to new TOAs
    plt.figure(figsize=(12,12))
    plt.subplot(4,1,1)
    plt.scatter(mytimes, toas, marker='.', color='k')
    plt.scatter(mytimes, mytimes, marker='.', color='tab:red')
    plt.xlabel('Original times')
    plt.ylabel('New times')
    plt.title('New and old TOAs')

    plt.subplot(4,1,2)
    plt.scatter(np.arange(0, Nobs), mytimes, marker='.', color='tab:blue', alpha=0.5)
    plt.scatter(np.arange(0, Nobs), toas, marker='.', color='tab:orange', alpha=0.5)
    plt.xlabel('Number')
    plt.ylabel('Time (days)')
    plt.title('New and old TOAs')

    plt.subplot(4,1,3)
    plt.plot(np.arange(0, Nobs), (toas-mytimes)*86400*omgc_0/(2*np.pi), marker='.', color='k')
    plt.xlabel('Number')
    plt.ylabel('Change (cycles)')
    plt.title('(New toas - old toas)*f0')

    plt.subplot(4,1,4)
    plt.plot(np.arange(0, Nobs-1), mytimes[1:]-mytimes[:-1], marker='.', color='tab:blue')
    plt.plot(np.arange(0, Nobs-1), toas[1:]-toas[:-1], marker='.', color='tab:orange')
    plt.xlabel('Number')
    plt.ylabel('Timestep (days)')
    plt.title('Old and new dts')

    plt.tight_layout()
    plt.savefig(f"{out_directory}/{tag}_toa_cadence_adjustments_true.png")
    plt.close()
...
def run(params):
    simulate_toas_cadence(params.tau, params.r, params.sigmac, params.sigmas, 
                          params.lag, params.phi_0, params.input_parfile, 
                          params.input_timfile, params.output_parfile, 
                          params.output_timfile, params.output_freqfile, 
                          params.out_directory, params.tag)
...
if __name__=="__main__":
    ...
    parser.add_argument("--input_parfile", default=None, type=str, help="par file for simulating with observing cadence")
    parser.add_argument("--input_timfile", default=None, type=str, help="tim file for simulating cadence")
