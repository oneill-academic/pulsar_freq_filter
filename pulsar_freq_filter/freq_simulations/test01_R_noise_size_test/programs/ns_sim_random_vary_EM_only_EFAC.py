...
def run(params):
    ...
    if not params.resume_run:
        ...
        plt.subplot(2,1,1)
        plt.plot(times, data, marker='.', linestyle=None, color="b")
        plt.xlabel("times")
        plt.ylabel("data")
        plt.title("Measurements over time")

        plt.subplot(2,1,2)
        plt.plot(times, data-omgc_0-omgc_dot*times, marker='.', linestyle=None, color="b")
        plt.xlabel("times")
        plt.ylabel("data")
    ...
    #This section runs the method using an EFAC parameter.
    #Set up the Kalman filter
    Nobs = len(times)
    design = np.asarray([1., 0.]).reshape(1, 2)
    measurement_cov = np.asarray([np.eye(1)*R_out for ii in range(Nobs)]).T
    model = TwoComponentModel(times, data.reshape((Nobs, 1)), measurement_cov, design, param_map)
    likelihood = KalmanLikelihood(model)

    #Get an estimate for omgc_dot by fitting a linear trend
    p, V = np.polyfit(times, data, 1, w=1/np.sqrt(np.squeeze(measurement_cov)), cov=True)
    ...
    priors['omgc_0'] = bilby.core.prior.DeltaFunction(data[0], name='omgc_0', latex_label='$\\Omega_{\\rm{c}, 0}$')
    ...
    result = bilby.run_sampler(likelihood, priors, sampler='dynesty',
                               ...
                               label=f'{params.tag}_EFAC', check_point_plot=False)

    #Make plots of the sampler results
    samples = result.posterior.to_numpy()[:, :7].copy()
    samples[:, 0:4] = np.log10(samples[:, 0:4])
    samples[:, 6] = np.log10(R_out)+np.log10(samples[:, 6])
    print("samples[-1] =", samples[-1])
    print("10**samples[-1] =", 10**samples[-1])
    print("measurement_cov =", measurement_cov)
    print("measurement_cov_new =", measurement_cov*10**samples[-1, -1])
    labels = ['$\\tau_{\\rm{s}}/\\tau_{\\rm{c}}$', '$\\tau$', '$\\sigma_{\\rm{c}}^2/I_{\\rm{c}}^2$',
              '$\\sigma_{\\rm{s}}^2/I_{\\rm{s}}^2$', '$\\langle\\Omega_{\\rm{c}} - \\Omega_{\\rm{s}}\\rangle$',
              '$\\langle \\dot\\Omega_c\\rangle$', '$R_{out} \\times EFAC$']
              #'$\\langle \\dot\\Omega_c\\rangle$', 'EFAC']
    fig = corner(samples, truths=[np.log10(taus/tauc), np.log10(tau), np.log10(sigmac**2), np.log10(sigmas**2), lag, omgc_dot, np.log10(R_in)],
                 range = [(np.log10(r_min), np.log10(r_max)), (np.log10(tau_min), np.log10(tau_max)),
                          ...
                          (lag_min, lag_max), (omgc_dot_min, omgc_dot_max), (np.log10(R_out)+np.log10(EFAC_min), np.log10(R_out)+np.log10(EFAC_max))],
    ...
    plt.savefig(f'{params.out_directory}/{params.tag}_corner_EFAC.png')
    plt.close()
    ...
    plt.subplot(5,2,9)
    plt.hist(samples[:, 6], 50, density=True, facecolor="k", alpha=0.4)
    plt.xlabel("EFAC")

    plt.suptitle("Recovered parameters")
    plt.tight_layout()
    plt.savefig(f"{params.out_directory}/{params.tag}_param_hists_EFAC.png")
    plt.close()



    #This section runs the method without using an EFAC parameter.
    #Set up the Kalman filter
    Nobs = len(times)
    design = np.asarray([1., 0.]).reshape(1, 2)
    measurement_cov = np.asarray([np.eye(1)*R_out for ii in range(Nobs)]).T
